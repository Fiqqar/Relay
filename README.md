...
> **Windows note:** pip installs the `relay` script into Python's `Scripts` directory.
> If `relay` isn't found in a new terminal, add that directory to your user `PATH`:
>
> ```powershell
> [Environment]::SetEnvironmentVariable(
>   'Path',
>   [Environment]::GetEnvironmentVariable('Path','User') + ';' +
>   'C:\Users\<you>\AppData\Local\Packages\PythonSoftwareFoundation.Python.3.11_*\LocalCache\local-packages\Python311\Scripts',
>   'User')
> ```
...
The `--provider` flag overrides `RELAY_AI_PROVIDER`. A TOML config file is a planned addition.
